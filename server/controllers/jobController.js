...
jobController.getFeed = async (req, res, next) => {
  try {
    ...
    await fetch(fullURI)
      ...
      .catch((err) =>
        console.log(`error occurred during GET request in getFeed: ${err}`)
      );
...
jobController.getJobs = async (req, res, next) => {
  try {
    ...
    const interestedIn = await Promise.all(
      await board1.interestedIn.map(async (jobId) => {
    ...
    const appliedFor = await Promise.all(
      await board1.appliedFor.map(async (jobId) => {
        return await models.Job.findOne({ _id: jobId });
      })
    );

    const upcomingInterviews = await Promise.all(
      await board1.upcomingInterviews.map(async (jobId) => {
        return await models.Job.findOne({ _id: jobId });
      })
    );
    const offers = await Promise.all(
      await board1.offers.map(async (jobId) => {
        return await models.Job.findOne({ _id: jobId });
      })
    );
    const archived = await Promise.all(
      await archivedIDs.map(async (jobId) => {
        return await models.Job.findOne({ _id: jobId });
      })
    );
    ...
    return next({
      ...
      message: {
        err:
          'An error occurred in jobController.getAllJob. Check server for more details',
      },
    });
  }
};
  ...
  try {
    ...
    models.Job.create({ ...newJob }, async (err, result) => {
      ...
      await models.User.findOneAndUpdate(
        { _id: userId },
        { $addToSet: { 'boards.0.interestedIn': result._id } },
        (err, newUser) => {
    ...
    return next({
      ...
      message: {
        err:
          'An error occurred in jobController.createJob. Check server for more details',
      },
    });
  }
};
...
jobController.moveJob = async (req, res, next) => {
  try {
    const {
      ...
      newIndex,
      boardIndex = 0,
    } = req.body; // this might be really tricky...
    ...
    await models.User.findOneAndUpdate(
      { _id: userId },
      { [previousColumnField]: previousColumn, [newColumnField]: newColumn },
      (err, newUser) => {
    ...
    return next({
      ...
      message: {
        err:
          'An error occurred in jobController.moveJob. Check server for more details',
      },
    });
  }
};
...
jobController.archive = async (req, res, next) => {
  try {
    ...
    await models.User.findOneAndUpdate(
      { _id: userId },
      { $addToSet: { archived: jobId }, [previousColumnField]: previousColumn },
      (err, newUser) => {
        res.locals.user = newUser;
        return next();
      }
    ...
    return next({
      ...
      message: {
        err:
          'An error occurred in jobController.moveToArchived. Check server for more details',
      },
    });
  }
};
...
jobController.deleteJob = async (req, res, next) => {
  try {
    ...
    const { userId, column, index } = req.body;
    await models.Job.deleteOne({ _id: id });
    const user = await models.User.findOne({ _id: userId });
    user.boards[0][column].splice(index, 1);
    const changedColumn = user.boards[0][column];
    const key = `boards.0.${column}`;
    ...
    return next({
      log: `An error occurred in deleteJob ${err}`,
